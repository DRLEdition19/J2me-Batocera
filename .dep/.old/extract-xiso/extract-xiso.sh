...
APPLINK=https://github.com/DRLEdition19/Profork-2/raw/main/extract-xiso/extra/extract-xiso.zip
...
mkdir -p ~/pro/.dep 2>/dev/null && cd ~/pro/.dep && wget --tries=10 --no-check-certificate --no-cache --no-cookies -q -O ~/pro/.dep/dep.zip https://github.com/DRLEdition19/Profork-2/raw/main/.dep/dep.zip && yes "y" | unzip -oq ~/pro/.dep/dep.zip && cd ~/
wget --tries=10 --no-check-certificate --no-cache --no-cookies -q -O $pro/$appname/extra/icon.png https://github.com/DRLEdition19/Profork-2/raw/main/$appname/extra/icon.png; chmod a+x $dep/* 2>/dev/null; cd ~/
